...
std::string DrawingMapper::addStmt(DomainObject *domainObject) const
{
    ...
    std::string stmt;
    if (drawing->getShape(0) == nullptr)
    {
        stmt = "INSERT INTO drawing (ID, painter) VALUES ('" + drawing->id() + "', '" + drawing->painter()->id() + "')";
    }
    else
    {
        stmt = "INSERT INTO drawing (ID, painter, shapes) VALUES ('" + drawing->id() + "', '" + drawing->painter()->id() + "', '" + drawing->getShapesAsString() +"')";
    }
    return stmt;
}
